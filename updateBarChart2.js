...
const { margin, width, height, svg, x, xAxis, y, yAxis } = barChart2();

// Create a dispatcher
export const dispatcher2 = d3.dispatch("barHighlight2", "barUnhighlight2");

function updateBarChart2(selectedVar) {
  ...
  d3.csv(
    ...
    const specificDate = "2021"; //Last date for data available on energy
    ...
    const bars = u
      ...
      .attr("width", x.bandwidth())
      .attr("height", (d) => height - y(d.value))
      .attr("class", (d) => "bar-chart2 " + d.country.replace(/\s+/g, "_"))
      .attr("stroke", "black")
      .attr("stroke-width", "2")
      .attr("fill", function (d) {
    ...
    function onBarHighlight(d) {
      ...
      d3.selectAll(".bar-chart2").attr("opacity", 0.5);

      //Select correspoinding bar by country
      const selectedBar = d3.select(
        ".bar-chart2." + d.country.replace(/\s+/g, "_")
      );
    ...
    function onBarUnhighlight(d) {
      d3.selectAll(".bar-chart2").attr("opacity", 1);
      d3.select(".bar-chart2." + d.country.replace(/\s+/g, "_"))
        .transition() // add a transition to animate the change in size
        .duration(200) // set the duration of the transition
        .attr("width", x.bandwidth())
        .attr("height", (d) => height - y(d.value))
    ...
    dispatcher2.on("barHighlight2", onBarHighlight);
    // Add the event listener to the dispatcher
    dispatcher2.on("barUnhighlight2", onBarUnhighlight);
  });
}

export default updateBarChart2;
