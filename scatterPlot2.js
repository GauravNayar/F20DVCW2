...
export const scatDispatcher2 = d3.dispatch(
  "scatHighlight2",
  "scatUnhighlight2"
);
...
function scatterPlot2() {
  ...
  d3.csv(
    ...
    svg
      ...
      .text("Renewable Energy Consumption(Log Scale)");
    ...
    svg
      ...
      .attr("dy", "1em")
      .style("text-anchor", "middle")
      .text("Renewable Energy Share(Log Scale)");
    ...
    const myCircle = svg
      ...
      .attr("r", 8)
      .attr("class", (d) => "scatter-plot2 " + d.country.replace(/\s+/g, "_"))
    ...
    function onScatHighlight(d) {
      ...
      const selectedScat = d3.select(
        ".scatter-plot2." + d.country.replace(/\s+/g, "_")
      );
    ...
    function onScatUnhighlight(d) {
      ...
      d3.select(".scatter-plot2." + d.country.replace(/\s+/g, "_"))
        .transition() // add a transition to animate the change in size
        .duration(200) // set the duration of the transition
        .attr("r", 8)
        .style("stroke", null);
    ...
    scatDispatcher2.on("scatHighlight2", onScatHighlight);
    // Add the event listener to the dispatcher
    scatDispatcher2.on("scatUnhighlight2", onScatUnhighlight);
  });
}
